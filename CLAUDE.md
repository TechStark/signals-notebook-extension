...
1. `src/options/options.ts` lets the user add any number of hosts — a literal host (`my-instance.signalsnotebook.com`) or a subdomain wildcard (`*.signalsnotebook.com`) — validated by `isValidSnbHost`, and calls `chrome.runtime.sendMessage({ type: 'REQUEST_HOST_PERMISSION', host })` for each one individually.
...
- **`popup/`**, **`options/`** — each is its own HTML document/entry, built with plain DOM APIs (no framework). `options/` is where SNB hosts are added/removed and permissions are requested.
